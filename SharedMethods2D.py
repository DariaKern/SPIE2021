...
def get_organized_data2D(path, DIMENSIONS, isSegmentation=False):
    ...
    if isSegmentation:
        data = np.zeros((number_of_files, DIMENSIONS[0], DIMENSIONS[1], DIMENSIONS[2]), dtype=np.bool)  # define y array
    else:
        data = np.zeros((number_of_files, DIMENSIONS[0], DIMENSIONS[1], DIMENSIONS[2]), dtype=np.uint16)  # define X array
