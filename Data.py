...
def crop_out_bb(img, box_path):
    ...
    img_arr = img_arr[x1:x2, y1:y2, z1:z2]
...
def resample_file(sitk_img, target_img_depth, target_img_height, target_img_width):
    ...
    vox_width = old_spacing[0]
    vox_height = old_spacing[1]
    vox_depth = old_spacing[2]
...
def crop_out_bbs(dict_files, dict_box_paths, save_path, organ=None):
    ...
    for key in sorted(dict_files.keys()):
        ...
        orig_img = dict_files[key]
        box_path = dict_box_paths[key]

        # crop out box area
        result_img_arr = crop_out_bb(orig_img, box_path)
        ...
        result_img = nib.Nifti1Image(result_img_arr, orig_img.affine, orig_img.header)
            ...
            for y in range(nib_file_arr.shape[1]):
                for z in range(nib_file_arr.shape[2]):
                    if nib_file_arr[x][y][z] > 0:
                        width = x + int(vox_bb[0])
                        height = y + int(vox_bb[2])
                        depth = z + int(vox_bb[4])
                        result_img_arr[width, height, depth] = nib_file_arr[x][y][z]
